...
typedef RT_MODEL_template_hardware_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * template_hardware(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(template_hardware_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(template_hardware_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: template_hardware -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(template_hardware_DW.HILReadEncoderTimebase_Task,
        (t_clock) template_hardware_P.HILReadEncoderTimebase_clock, 1000.0, -1)
        ;
      if (result == 0) {
