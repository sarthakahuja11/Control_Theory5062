...
typedef RT_MODEL_model_5_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * model_5(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(model_5_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(model_5_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://model_5:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: model_5 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(model_5_DW.HILReadEncoderTimebase_Task, (t_clock)
        model_5_P.HILReadEncoderTimebase_clock, 1000.0, -1)
        ;
      if (result == 0) {
